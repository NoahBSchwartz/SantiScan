...
//const char* ssid = "HOME-6922";
//const char* password = "7495CEEE8546429B";
const char* ssid = "STAFF1920";
const char* password = "K1n3t1cF0rc3";
...
int qualityconfig = 5;
int buffersconfig = 3;
int avi_length = 1;            // how long a movie in seconds -- 1800 sec = 30 min
int frame_interval = 1;          // record at full speed
...
#include <WebServer.h>
const int port = 8080;  // the port number to listen on
...
//Noah
  ...
  esp_task_wdt_init(900000000000000, false);
  //Serial.begin(115200);
  Serial.println("\n\n---");
...
void the_camera_loop (void* pvParameter) {
  ...
  while (1) {
    ...
    //Noah
    while ((digitalRead(13) == 1) && (noah_start == 0))
      {
          Serial.println(WiFi.localIP());
      }
    noah_start = 1;
    ...
    //Noah
      start_record = 0;
      frame_cnt = 1000000000000;             // start recording again on the next loop
...
/*
  ...
  if (client) {
    ...
    while (file) {
      ...
      if (strstr(file.name(), ".avi")) {
        Serial.println(file.name());
        Serial.println("Sending video file...");
        File videoFile = SD_MMC.open(file.name(), FILE_READ);
        if (videoFile) {
          while (videoFile.available()) {
            uint8_t buffer[1024];
            int bytesRead = videoFile.read(buffer, sizeof(buffer));
...
//wpksw
          Serial.println("Video file sent.");
        } else {
